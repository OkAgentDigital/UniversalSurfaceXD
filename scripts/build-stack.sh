...
# Part of DevStudio / Universui Sub-Sonic-Stack system
# ============================================================
...
#!/bin/bash
# Universui Stack Launcher
# Generated by DevStudio Stack Builder

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export UNIVERSUI_STACK="$(basename "$SCRIPT_DIR")"
export UNIVERSUI_MODE="${UNIVERSUI_MODE:-gui}"

echo "🚀 Starting Universui stack: $UNIVERSUI_STACK"
echo "   Mode: $UNIVERSUI_MODE"

# Check for main entry point
if [ -f "$SCRIPT_DIR/node_modules/@universui/core/dist/main.js" ]; then
    node "$SCRIPT_DIR/node_modules/@universui/core/dist/main.js" "$@"
elif [ -f "$SCRIPT_DIR/node_modules/@universui/core/index.js" ]; then
    node "$SCRIPT_DIR/node_modules/@universui/core/index.js" "$@"
elif command -v universui &> /dev/null; then
    universui --stack "$UNIVERSUI_STACK" "$@"
else
    echo "❌ No Universui executable found"
    echo "   Install with: npm install -g @universui/core"
    exit 1
fi
LAUNCHER
